...
export const findTestimonials = async (): Promise<ITestimonial[]> => {
  try {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/contact/testimonials`)

    return response.data;
  } catch (error) {
    // @ts-ignore
    throw new Error(error.response.data.message || 'Erro ao buscar testimonials')
  }
}
